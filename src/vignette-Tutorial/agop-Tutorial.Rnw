...
(in many practical situations we often choose $\Ival=[0,1]$ or $\Ival=[0,\infty]$).
...
we write $\vect{x}\le\vect{y}$
iff for all $i$ it holds $x_i\le y_i$

$(n\ast c) = (c,\dots,c)\in\IvalPow{n}$
...
in a matrix (column-/row-wise.... col/rownames....) apply()....
...
pord_weakdom <- function(x, y)
   length(x) <= length(y) && all(sort(x, decreasing=TRUE) <= sort(y, decreasing=TRUE)[1:length(x)])

ord <- matrix(NA, nrow=length(ex1), ncol=length(ex1))
colnames(ord) <- names(ex1)
rownames(ord) <- names(ex1)

for (i in 1:length(ex1)) {
   for (j in 1:length(ex1)) {
      ord[i,j] <- pord_weakdom(ex1[[i]], ex1[[j]])  
   }
}
@

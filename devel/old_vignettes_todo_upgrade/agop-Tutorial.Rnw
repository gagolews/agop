...
%\VignetteKeyword{aggregation}
%\VignetteKeyword{preorder}
%\VignetteKeyword{mcdm}
...
\bigskip
\begin{definition}\label{Def:cotnorm}
...
\bigskip
\begin{definition}\label{Def:fneg}
...
For any t-conorm $S$ and all $x,y$ it holds $S_\mathrm{M}(x,y)\le S(x,y)\le S_\mathrm{D}(x, y)$.

...
function.
On the other hand,
the \emph{transitive reduction} of $R$, see \cite{AhoGareyUllman1972:transitivereduction}
...
A transitive reduction is particularly useful when drawing Hasse
diagrams of preoredered sets.
...
To draw the Hasse diagram, we base on transitive and
reflexive reduction of the graph.
\index{\Rfunc{rel\_reduction\_transitive()}}
\index{\Rfunc{rel\_reduction\_reflexive()}}

<<ex1C,dependson='ex1B'>>=
require(igraph)
hasse <- graph.adjacency(rel_reduction_reflexive(rel_reduction_transitive(ord)))

...
has reminded underground till only recent (\dots).'' \cite[p.~xiii]{GrabischETAL2009:aggregationfunctions}
\end{quotation}
...
are sometimes called \emph{averaging functions}, cf.~\cite{CITATION_NEEDED}. %%%%%%%%%%%%%%%%!!!!!!!!!!!!!!!!!!!!! Aniu, cytowanko
...
\medskip
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% MOZE ZACYTOWAC SKAD TA DEFINICJA?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  W MESIAR/MESIAROVA-ZEMANKOVA, 2011 JEST NA PEWNO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  PODANE ZRODLO
\begin{definition}
...
It may easily be seen that each additive operator fulfiling
the lower strong boundary condition is also modular
(i.e.~modularity is more general than additivity),
% because ....................................................    %%%%% ANIU, CWICZONKO DLA CIE :)
...
\item The \emph{ordered weighted averaging operator}
associated with $\vect{w}$, %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CYTOWANIE: Yager
...
%%%%%%% In fact, F is idempotent if and only if............................................ ?????
Moreover, for $\vect{w}=(n\ast 1/n)$, $\func{WAM}_\vect{w}$ 
defines the arithmetic mean (\Rfunc{mean()} in \R).
%%%%%%%%%%% WSPOMNIEC O SREDNIEJ UCIETEJ \Rfunc{mean(x, trim=...)} JAKO CIEKAWYCH PRZYKLADACH
...
with $\vect{w}$, $\func{WMax}_\vect{w}: \IvalPow{n}\to\Ival$, %%%%%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CYTOWANIE: Dubois, Prade 1988 bodaj
 is defined as 
\[\func{WMax}_\vect{w}(\vect{x})=\bigvee_{i=1}^{n}(w_{i}\wedge x_{i}).\]

\item The \emph{ordered weighted maximum}  associated 
with $\vect{w}$,
$\func{OWMax}_\vect{w}: \IvalPow{n}\to\Ival$, is defined as 
\[\func{OWMax}_\vect{w}(\vect{x})=\bigvee_{i=1}^{n}(w_{i}\wedge x_{(i)}),\]
with $x_{(1)}\leq\dots\leq x_{(n)}$ and $w_{1}\geq w_{2}\geq\dots\geq w_{n}$.   %%%%%%%% ZMIENIC NA x_{\{i\}} - zmienia sie warunki
\end{enumerate}
\end{definition}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% JAKI JEST ZWIAZEK M/Y OWMin a OWMax? to ta sama klasa!
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CZY KTORES Z TYCH FUNKCJI SA MINITYWNE/MAKSYTYWNE? :)
...
.............
%%%%%%%%%%%%%%%%%%% CHYBA BYM SOBIE DAROWAL L, S i I statystyki
%%%%%%%%%%%%%%%%%%% OD RAZU UDERZYC w QUASI :) I tylko wspomniec
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CZY SA ADDYTYWNE??????
%%%%%%%%%%%%%%%%%%% DLA JAKICH TROJKATOW JEST TO AGGREGATION FUNCTION?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% JAKIE WARUNKI MUSI SPELNIAC TROJKAT????????????
...
%%%%%%%%% JAK TO SIE MA DO MAXITIVITY?
%%%%%%%%%%%%%%%%%%%%%%%% BEZ STRATY OGOLNOSCI JAKIE WARUNKI???????
...
%%%%%%%%%%%%%% IT HAS BEEN SHOWN IN GAGOLEWSKI2012 THAT......... REKLAMA!

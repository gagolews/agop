...
we write $\vect{x}\le\vect{y}$ if and only 
...
with vectors of unequal length is \R).
...
Please note 
that the \textsf{dimnames} attribute of the matrix may define
...
(parameter \textsf{MARGIN} set to 1 and 2, respectively).
...
is called an \emph{(extended) aggregation operator}
...
\bigskip
Here are some interesting properties of aggregation operators.
Later on we will characterize the classes of aggregation operators
that fulfill them.
...
i.e.~it may be defined solely using order statistics.
Intuitively, $\func{F}$ is symmetric if its value is independent
...
\end{definition}
Please note that if $a\leq0$, then for additivity $\func{F}$ 
have to be such that $\func{F}(\vect{0})=0$.
...
(cf.~\cite{BeliakovETAL2007:aggregation,GrabischETAL2009:aggregationfunctions,MesiarMesiarova2011:orderedmodular}) if:
\[
(\forall n\in\mathbb{N})\ (\forall \vect{x},\vect{y}\in\IvalPow{n})\ 
...
for additive aggregation operator $\func{F}$.
...
that represent solutions to some very interesting practical issues.
...
according to the \emph{products}
(e.g.~scientific articles, works, books, artifacts) each entity has produced.
It is assumed that for each of the products 
some quality or popularity \emph{rating} (e.g.~the number of articles'
citations, the number of sold book copies etc.) is given.
...
for example scientometrics, webometrics, marketing,
...
papers, which in turn themselves receive citations.
...
Let us now consider some properties that can  be useful while aggregating vectors 
of varying lengths. This model described situation when a~``producer'' whose quality
...
\emph{zero-insensitive} aggregation operator, denoted $\func{F}\in
\Property{a0}$, if
...
It is easily seen that $\Property{a0}\cap \Property{nd}\subseteq
\Property{am}\cap \Property{nd}$,
where $\Property{am}$ denotes arity-monotonic aggregation operators,
see \cite{GagolewskiGrzegorzewski2010:ipmu},
such that
$(\forall \vect{x},\vect{y}\in\IvalAnyPow)\ \func{F}(\vect{x})\le
\func{F}(\vect{x},\vect{y}).$
...
$\func{F}\in \mathcal{E}(\Ival)$ is \emph{$F$-insensitive},
denoted $\func{F}\in \Property{F0}$, if
\begin{equation*}
...
\emph{$F$+sensitive}, denoted $\func{F}\in \Property{F+}$,
if
\begin{equation*}
...
Originally, they were defined for fixed-length vector lengths
and for $\Ival=[0,1]$.
...
\item The \emph{ordered weighted averaging operator} (cf.~\cite{Yager:OWA})
...
In fact, F is idempotent if and only if $\sum_{i=1}^{n}w_{i}=1$.
Moreover, for $\vect{w}=(n\ast 1/n)$, $\func{WAM}_\vect{w}$ 
defines the arithmetic mean (\Rfunc{mean()} in \R).
\emph{Truncated mean} is an interesting example of arithmetic mean. The main differences  
...
In \Rfunc{agop} $\func{OWA}$ operator is available as \Rfunc{owa()}.

<<owa>>=
owa(c(1,3,5,2),rep(1,4)) 
@
...
\item The \emph{ordered weighted maximum} (cf.~\cite{DuboisETAL1988:makingdecision})
associated with $\vect{w}$,
$\func{OWMax}_\vect{w}: \IvalPow{n}\to\Ival$, is defined as 
\[\func{OWMax}_\vect{w}(\vect{x})=\bigvee_{i=1}^{n}(w_{i}\wedge x_{\{i\}}),\]
with $x_{\{1\}}\geq\dots\geq x_{\{n\}}$ and $w_{1}\geq w_{2}\geq\dots\geq w_{n}$.   
% DONE % ZMIENIC NA x_{\{i\}} - zmienia sie warunki
\end{enumerate}
\end{definition}

\Rfunc{agop} implementation: \Rfunc{owmax()}
...
\[\func{WMin}_\vect{w}(\vect{x})=\bigwedge_{i=1}^{n}((1-w_{i})\vee x_{i}).\]

\item The \emph{ordered weighted minimum} $\func{OWMin}_\vect{w}: \IvalPow{n}\to\Ival$ associated 
with the weight vector $\vect{w}$ is defined as 
\[\func{OWMin}_\vect{w}(\vect{x})=\bigwedge_{i=1}^{n}((1-w_{i})\vee x_{(i)}),\]
with $x_{(1)}\leq\dots\leq x_{(n)}$ and $w_{1}\leq w_{2}\leq\dots\leq w_{n}$.
\end{enumerate}
\end{definition}

It is clear to see that $\func{OWMax}$ operators fulfill maxitivity property and
$\func{OWMin}$ operators fulfill minitivity property. What is more,
interestingly it can be shown, cf.~\cite{GrabischETAL2009:aggregationfunctions}, 
that for each $\func{OWMax}$ operator there exist equivalent $\func{OWMin}$ 
operator and inveresly.


\bigskip
%%%%%%%%%%%%%%%%%%% INTRO - PRZECHODZIMY DO PRZYPADKU WEKTOROW O DOWOLNEJ DLUGOSCI
In the previous reasoning we include only vectors of fixed lengths. 
Let us now consider process of aggregation vectors of varying lengths. 
Here we present extensions of operators described above into functions
from $\IvalAnyPow$ to $\Ival$.
...
\begin{definition}
Let $\triangle=(f_{i,n})_{ i\in [n],n\in\Naturals}$ be a triangle of functions 
...
\[
\func{qL}_{\triangle}(\vect{x})=\sum_{i=1}^{n}\func{f}_{i,n}(x_{(n-i+1)}).
\]
\end{definition}
\bigskip

Please note that the quasi-L-statistic generalized $\func{OWA}$ operators, for
which we have $\func{f}_{i,n}(x)=c_{i,n}x$, $c_{i,n}\in[0,1]$ and 
$(\forall n)\ \sum_{i=1}^{n}c_{i,n}=1$. 

Lest assume for simplicity that $\Ival=[0,b]$.
Interestingly it has been shown (cf.~\cite{Gagolewski2012:ins}) that aggregation
operator $\func{F}:\IvalAnyPow\to\Ival$ fulfill symmetric modularity property if and only if 
$\func{F}$ is a nondecreasing quasi-L-statistic. What is more, in~\cite{Gagolewski2012:ins}
we may find that $\func{qL}_{\triangle}$ is nondecreasing
if and only if there exist $\triangledown=(\func{g}_{i,n})_{i\in[n],n\in\Naturals}$ such that
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ZDEFINIOWAC UZYWAJAC ith GREATEST a nie SMALLEST VALUE
%dlaczego?
\bigskip
\begin{definition}
...
\[
\func{qS}_{\triangle}(\vect{x})=\bigvee_{i=1}^{n}\func{f}_{i,n}(x_{(n-i+1)}),
\] for any $\vect{x}\in\IvalAnyPow$.
\end{definition}
\bigskip

It is easily seen that quasi-S-statistic generalized $\func{OWMax}$ 
operators for which we have $\func{f}_{i,n}(x)=x\wedge c_{i,n}$, $c_{i,n}\in\Ival$
and $(\forall n)\ \bigvee_{i=1}^{n}c_{i,n}=b$.

Moreover, it has been shown (cf.~\cite{Gagolewski2012:ins}) that every symmetric maxitive 
aggregation operator is a nondecreasing quasi-S-statistic (i.a. without loss of generality 
we may assume that nondecreasing quasi-S-statistic is generated by triangle of functions
in which $(\forall n)\ (\forall i\in[n])\ \func{g}_{i,n}$ is nondecreasing, 
$(\forall n)\ (\forall i\in[n])\ \func{g}_{i,n}(a)=\func{g}_{n,n}(a)$ 
and $(\forall n)\ \func{g}_{1,n}\preceq\dots\preceq\func{q}_{n,n}$).

\bigskip
\begin{definition}
The \emph{quasi-I-statistic} for a given triangle of functions 
$\triangle=(f_{i,n})_{ i\in [n],n\in\Naturals}$
is a function $\func{qI}_{\triangle}:\IvalAnyPow\to\Ival$ such that 
\[
\func{qI}_{\triangle}(\vect{x})=\bigwedge_{i=1}^{n}\func{f}_{i,n}(x_{(n-i+1)}),
\] for any $\vect{x}\in\IvalAnyPow$.
\end{definition}
\bigskip

Quasi-I-statistic is generalizations of well-known $\func{OWMin}$ operators,
for which we have $\func{f}_{i,n}(x)=x\vee c_{i,n}$, $c_{i,n}\in\Ival$ and 
$(\forall n)\ \bigwedge_{i=1}^{n}c_{i,n}=a$.

Similar like in above, it has been shown (cf.~\cite{Gagolewski2012:ins}) that every 
symmetric minitive 
aggregation operator is a nondecreasing quasi-I-statistic (i.a. without loss of generality 
we may assume that nondecreasing quasi-S-statistic is generated by triangle of functions
in which $(\forall n)\ (\forall i\in[n])\ \func{g}_{i,n}$ is nondecreasing, 
$(\forall n)\ (\forall i\in[n])\ \func{g}_{i,n}(b)=\func{g}_{n,n}(b)$ 
and $(\forall n)\ \func{g}_{1,n}\preceq\dots\preceq\func{q}_{n,n}$).

\bigskip
Many times we consider L--, S-- and I-- statistics, a special cases
of defined above quasi-statistics, generated by a triangle of coefficient 
(i.a.~\emph{triangle of coefficients} is a sequence 
$\triangle=(c_{i,n}\in\Ival: i\in [n], n\in\Naturals)$, cf~\cite{CalvoMayer1999:aggregation}).
%%%%%%%%%%%%%%%%%%%% TRIANGLE OF COEFFICIENTS - TO JUZ UZYWALA TOMASA CALVO i GASPAR MAYOR, JOURNAL=TATRA MOUNTAINS COSTAM
In fact, \emph{L-statistics} is a \emph{quasi-L-statistics} for which we have
$\func{f}_{i,n}(x)=c_{i,n}x$. Similarly, by setting 
$\func{f}_{i,n}(x)=x\wedge c_{i,n}$ we obtain \emph{S-statistic} from \emph{quasi-S-statistics}, and
by setting $\func{f}_{i,n}(x)=x\vee c_{i,n}$ we obtain \emph{I-statistic} from \emph{quasi-I-statistics}.
...
\bigskip
In~\cite{Gagolewski2012:ins} it has been shown, that 
intersection of every two from 
symmetric maxitive, minitive and modular operators classes is the same.
Based on this result \emph{OM3} class (symmetric maxitive, minitive and modular 
aggregation operators) was proposed. In~\cite{CenaGagolewski2013:agop1,CenaGagolewski2013:agop2}
definition and basic properties of OM3 operators are given.
...
%%%%%%%%%%%% jaki poziom szczegolowosci chcesz zachowac? Napisz mi w komentarzu 
%%%%%%%%%%%% to dopisze ewentualnie 
%%%%%%%%%%%%%%%%%%%%%% BARDZO SZCZEGOLOWO! WSZYSTKO, CO WIEMY
%%%%%%%%%%%%%%%%%%%%%% NIECH TO BEDZIE ZESTAW NAJOGOLNIEJSZYCH WYNIKOW, KTORE ZNAMY
...
%nie za duzo jedno to? with respect to to?
...
Hirsch's index is a zero-insensitive impact function, denoted $\func{H}\in
\Property{a0}$, which means that 
for each $\vect{x}\in\IvalAnyPow$ it holds
$\func{H}(\vect{x},0)=\func{H}(\vect{x})$.
...
\[
\func{l}_{p}(\vect{x})=\mathsf{arg}\sup_{(u,v)}{\{uv: \func{e}^{p,u,v}\trianglelefteq\vect{x}\}},
\]
...
The MaxProd-index (cf.~\cite{Kosmulski2007:maxprod}) given  by the equation
...
In \package{agop} MaxProd-index is implemented as \Rfunc{index\_maxprod()}.
...
properly transformed input vectors.....                               %%%%%%% JAK UZYSKAC H2 z QUASI-S?
...
Theory of aggregation looks at the aggregation operators
...
In such case we assume that input data to be aggregated
are in fact realizations of some random samples.

In probability, an aggregation operator
is simply called a \emph{statistic} (formalism......)
%%% co rozumiesz przez formalism? jakies definicje wzory twierdzenia?
%%%%%%%%%%%%%%%%%%%%% Nie, no ogolnie, co to jest statystyka :)

Let $(X_1,\dots,X_n)$ i.i.d.~$F$, where $\mathrm{supp}\,F = \Ival$.
In the modeling of social phenomena, if $F$ is continuous,
...
The estimation 
procedure has been implemented in \package{agop} as TO DO: 
\Rfunc{pareto2.mleksestimate()}....
...
In this case one of the estimators worth noting 
(and often better than MLE) was proposed
...
cf~\ref{DavidNagaraja2003:orderstatistics}. To this end, first define
\[

...
\footnote{\os{Windows} users should have \textsf{Rtools} installed, see \href{http://cran.r-project.org/bin/windows/Rtools/}%
...
\subsection{Dispersion Functions}\label{Sec:DispersionFunctions}


Classically, in the theory of aggregation we consider
the broadly-conceived averaging functions and logical connectives.
However, one often needs a very different kind of a proper
synthesis of multidimensional numeric data into a single number.
In \cite{GagolewskiXXXX:dispersion} an axiomatization of
\emph{dispersion functions}, which may be used to measure data
variability, spread, or scatter, was proposed.



Given $\vect{x},\vect{x}'\in\Reals^n$,
we write $\vect{x}\preccurlyeq\vect{x}'$
and say that $\vect{x}$ \emph{has not greater spread than} $\vect{x}'$,
if and only if for all $i,j=1,\dots,n$ it holds
\begin{equation}
x_i\ge x_j \Longrightarrow x_i-x_j\le x_i'-x_j'.
\end{equation}
It is easily seen that $\preccurlyeq$ is a preorder on $\Reals^n$,
i.e.~a relation that is reflexive and transitive,
and that it is not necessarily total, i.e.~not all vectors
are comparable with each other, see also Sec.~\ref{Sec:VectorSpread}.

\begin{definition}\label{Def:DispersionOperator}
A \emph{dispersion function} is an aggregation operator
$\func{V}:\Reals^n\to[0,\infty]$ such that:
\begin{enumerate}
 \item[(d1)] For any $c\in\Reals$
 it holds $\func{V}(n\ast c)=\func{V}(c,c,\dots,c)=0$,
 \item[(d2)] for each $\vect{x}\preccurlyeq\vect{x}'$
...
(see \Rfunc{var()}), standard veriation
...
handling.}
defined on finite sets $V=\{v_1,\dots,v_n\}$.
...
Table \ref{Tab:binrelprops} contains an overview of the most often
...
Antisymmetry & $(\forall i,j)$ $v_i R v_j$ and $v_j R v_i$ $\Rightarrow$ $v_i=v_j$ & TO DO \\
...
Total Preorder  & total ($\Rightarrow$ reflexive), transitive \\
Partial Order  & reflexive, antisymmetric, transitive \\
Total Order  & total ($\Rightarrow$ reflexive), antisymmetric, transitive \\
...
......................
\emph{transitive reduction}, see \cite{AhoGareyUllman1972:transitivereduction},
TO DO: \index{\Rfunc{rel\_reduction\_transitive()}}\Rfunc{rel\_reduction\_transitive()}
-- it is particularly useful when drawing Hasse diagrams of preoredered sets.
......................
...
\index{\Rfunc{rel\_reduction\_reflexive()}}\Rfunc{rel\_reduction\_reflexive()}
...
In general, a total closure and total reduction is not well-defined.
...
Moreover, it would have been a partial order (in general it's not),
...
\subsection{Weak Dominance Relation (for fixed arity)}


Consider the following relation on $\IvalAnyPow$.
For any $\vect{x}\in\IvalPow{n}$ and $\vect{y}\in\IvalPow{m}$ we write
% \begin{equation}\label{Eq:partialorderingtrianglelefteq}
$\vect{x}\le\vect{y}$ if and only if
$n = m\text{ and }x_{i}\le y_{i}$ for all $i=1,\dots,n$.
...
can be used to compare spread, scater, or variability of two numeric vectors,
see \cite{GagolewskiXXXX:dispersion}
with the $\preccurlyeq$ relation introduced
in Sec.~\ref{Sec:DispersionFunctions}
and \cite{GagolewskiXXXX:dispersion}.
...
To draw the Hasse diagram, it will be good to base on transitive reduction of
...
let's determine the \emph{fair total closure} of $\trianglelefteq$
...
print(ord_total)
hasse <- graph.adjacency(rel_reduction_transitive(ord_total))
set.seed(1234)
plot(hasse, layout=layout.fruchterman.reingold(hasse, dim=2))
@
...
However, it might be shown (see \cite{GagolewskiMesiarXXXX:integrals}
...
Interestingly, we have shown in \cite{GagolewskiMesiarXXXX:integrals}
...
see \cite{GagolewskiMesiarXXXX:integrals} for discussion.
...
\subsection{Interesting Dispersion Functions}
...
%     to .bashrc
\bibliography{gagolewski,gagolewski_temp,aggregation,probstat,%
r,bibliometrics,unsorted}

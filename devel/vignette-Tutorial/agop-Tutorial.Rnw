...
\func{F}(\vect{x}\stackrel{S}{\wedge}
\vect{y})=\func{F}(\vect{x})\wedge\func{F}(\vect{y}),\] where
$\vect{x}\stackrel{S}{\wedge}\vect{y}=(x_{(1)}\wedge y_{(1)},\dots,x_{(n)}\wedge
y_{(n)})$ for $\vect{x},\vect{y}\in\IvalPow{n}$.
\end{definition}

\begin{definition}
We call $\func{F}$ \emph{maxitive} if \[(\forall n)\ (\forall \vect{x},\vect{y}\in\IvalPow{n})\
\func{F}(\vect{x}\stackrel{S}{\vee}
\vect{y})=\func{F}(\vect{x})\vee\func{F}(\vect{y}),\] where 
$\vect{x}\stackrel{S}{\vee}\vect{y}=(x_{(1)}\vee
y_{(1)},\dots,x_{(n)}\vee y_{(n)})$ for $\vect{x},\vect{y}\in\IvalPow{n}$.
\end{definition}
...
\end{definition}
 ...
 (L-statistics)
\begin{definition}
The \emph{triangle of coefficients} 
\end{definition}

...
representative one is called \textit{aggregation}, and the numerical function 
performing this process is called \textit{aggregation function}. This simple definition
demonstrates the size of the field of application of aggregation: applied mathematics
(e.g. probability, statistics, decision theory), computer science (e.g. artificial 
intelligence, operation research), as well as many applied fields (economics and finance,
pattern recognition and image processing, data fusion, multicriteria decision making,
automated reasoning etc.).'' \cite{GrabischETAL2009:aggregationfunctions}
\end{quote}
...
Aggregation - what for?....... motivation....
...
\begin{definition}
We call $\func{F}$ \emph{modular} if \[(\forall n)\ (\forall \vect{x},\vect{y}\in\IvalPow{n})\ 
...
\textbf{Producers Assessment Problem} (PAP) concerns evaluations of a set of 
\textbf{producers} (e.g. scientists, artist, writers, craftsman) 
according to certain quality of their \textbf{products} (e.g. articles,
works, books, artifacts). We assume that each of products 
...
engineering, and its main interest if focused on constructing and analyzing 
aggregation operators which may be used in producers rating process. 
However, it is agreed that such functions should take into a count, two following 
aspects of producer quality:

\begin{itemize}
\item the ability to output highly-rated products
\item the overall productivity
\end{itemize}

Let us now consider PAP in context of scientometrics, where scientists are producers 
and articles are their products. 
...
cf.~\cite{CenaGagolewski2013:agop2}.

...
Transitive closure (minimal transitive $R'\supseteq R$)
...
for discussion), and then calculate the transitive closure.%
\index{\Rfunc{rel\_closure\_transitive()}}%
\index{\Rfunc{closure\_total\_fair()}}%

<<ex1D,dependson='ex1C'>>=
ord_total <- rel_closure_transitive(closure_total_fair(ord)) # a total preorder
print(ord_total)

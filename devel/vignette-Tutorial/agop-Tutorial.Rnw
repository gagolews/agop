...
space of vectors of arbitrary length, cf.~e.g.~\cite{CalvoMayor1999:aggregation,
...
\subsection{Dispersion Operators}

...TO DO... see \cite{Gagolewski2013:dispersion} ...
...
Of course, $\trianglelefteq$ is a preorder -- it
...
This preorder in \package{agop} may be
...
\subsection{Vector Spread}

...TO DO...

The functions \Rfunc{pord\_spread()} and \Rfunc{pord\_spreadsym()}
can be used to compare spread of two numeric vectors,
see \cite{Gagolewski2013:dispersion}...

This relation is used in the definition of dispersion operators,
see \cite{Gagolewski2013:dispersion}...
...
\subsection{Operations on Preorders}
...
\noindent
\package{agop} implementation: \Rfunc{wmax()} and \Rfunc{owmax()}.

<<owmax>>=
wmax(c(1,3,5,2), Inf) # no vectorization here!
...
\noindent
\package{agop} implementation: \Rfunc{wmin()} and \Rfunc{owmin()}.
...
if we set $\func{f}_{i,n}(x)=c_{i,n} x$, $c_{i,n}\in[0,1]$, and
$(\forall n)\ \sum_{i=1}^{n}c_{i,n}=1$.

Assume that $\Ival=[0,b]$.
Interestingly, it has been shown (\cite{MesiarMesiarova2011:orderedmodular},
cf.~also \cite{Gagolewski2012:ins})
that an aggregation
...
What is more, in~\cite{Gagolewski2012:ins}
...
if $\func{f}_{i,n}(x)=x\wedge c_{i,n}$, $c_{i,n}\in\Ival$
...
if $\func{f}_{i,n}(x)=x\vee c_{i,n}$, $c_{i,n}\in\Ival$ and
...
cf.~\cite{CalvoMayor1999:aggregation}).
...
\bigskip
Most interestingly, in~\cite{Gagolewski2012:ins} it has been shown that
...
and is available in \package{agop} by calling \Rfunc{index\_g()}.
...
However, it might be shown (see \cite{GagolewskiMesiar2013:fuzzyintbiblio}
...
\end{equation*}
and is available in \package{agop} by calling \Rfunc{index\_w()}.

Interestingly, we have shown in \cite{GagolewskiMesiar2013:fuzzyintbiblio}
...
\package{agop} implementation \Rfunc{index\_rp()}.
...
\package{agop} implementation \Rfunc{index\_lp()}.
...
see \cite{GagolewskiMesiar2013:fuzzyintbiblio} for discussion.

In \package{agop} the MAXPROD-index is implemented
in the \Rfunc{index\_maxprod()} function.
...
\subsection{Interesting Dispersion Operators}
...
is implemented in \package{agop}: \Rfunc{dpareto2()} gives the p.d.f.~\eqref{Eq:Pareto2PDF},
\Rfunc{ppareto2()} gives the c.d.f.~\eqref{Eq:Pareto2CDF},
\Rfunc{qpareto2()} calculates the quantile function, $F^{-1}$,
and \Rfunc{rpare\-to2()} generates random deviates.
...
\Rfunc{pareto2\_estimate\_mle()} with explicitly
...
In \package{agop} it is available as: \Rfunc{pareto2\_estimate\_mmse}.
...
\medskip
\package{agop} implementation: \Rfunc{pareto2\_test\_f()}.

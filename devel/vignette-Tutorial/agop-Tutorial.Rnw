...
$\triangle=(c_{i,n}\in\Ival: i\in [n], n\in\Naturals)$, cf~\cite{}).
...
$f_{i,n}(x)=c_{i,n}x$. Similarly, by setting 
...
%%%%%%%%%%%%%%%%%%%% W OGOLE WARTO ZERO_INSENSITIVE ORAZ F0, F+ DODAC DO *PAP*



\bigskip
In~\cite{Gagolewski2012:ins} it has been shown, that 
intersection of every two from class of
symmetric maxitive, minitive and modular aggregation is the same.
Based on this result class of symmetric maxitive, minitive and modular 
aggregation operators, \emph{OM3} operators, was proposed.

In~\cite{CenaGagolewski2013:agop1,CenaGagolewski2013:agop2} definition and
basic properties of OM3 operators are given.

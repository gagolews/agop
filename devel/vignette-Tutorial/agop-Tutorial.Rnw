...
that \textsf{dimnames} attribute for the matrix giving the row and column names. 
It value can be set as NULL (no names for columns and rows are given) 
or a list of length 2 (rows and columns names respectively).
Another simply way to set columns and row names is using \Rfunc{colnames()} 
and \Rfunc{rownames()} functions.

Function \Rfunc{apply()} evaluate the value of the given function on each row 
or column (row- or column-wise) of the given matrix (parameter \textsf{MARGIN} 
...
Vectors that are not of the same length can be store in the list, with 
possibly named elements. In that case functionality of \Rfunc{apply()}
functions is given by \Rfunc{lapply()} and \Rfunc{sapply()} functions.
...
\subsection{Examples of basic aggregation operators}
...
\emph{Hirsch index} is a zero-insensitive impact function, denoted $\func{H}\in

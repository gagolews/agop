...
\emph{spread measures}, which may be used to measure data
...
A \emph{spread measure} is an aggregation operator
...
\hline
\hline
Minimum & \index{tnorm\_minimum()}\Rfunc{tnorm\_minimum()} & $T_\mathrm{M}(x, y)=x\wedge y$ \\
Product & \index{tnorm\_product()}\Rfunc{tnorm\_product()} & $T_\mathrm{P}(x, y)=xy$ \\
Łukasiewicz & \index{tnorm\_lukasiewicz()}\Rfunc{tnorm\_lukasiewicz()} & $T_\mathrm{Ł}(x, y)=(x+y-1)\vee 0$ \\
Drastic & \index{tnorm\_drastic()}\Rfunc{tnorm\_drastic()} & $$T_\mathrm{D}(x, y)=\left\{
\begin{array}{ll}
0 & \text{if }x, y\in[0,1)\\
x\wedge y & \text{if }x=1\text{ or }y=1\\
\end{array}
\right.$$ \\
Fodor & \index{tnorm\_fodor()}\Rfunc{tnorm\_fodor()} & $$T_\mathrm{F}(x, y)=\left\{
\begin{array}{ll}
0 & \text{if }x+y\le 1\\
...
\hline
\hline
Maximum & \index{tconorm\_minimum()}\Rfunc{tconorm\_minimum()} & $S_\mathrm{M}(x, y)=x\vee y$ \\
Product & \index{tconorm\_product()}\Rfunc{tconorm\_product()} & $S_\mathrm{P}(x, y)=x+y-xy$ \\
Łukasiewicz & \index{tconorm\_lukasiewicz()}\Rfunc{tconorm\_lukasiewicz()} & $S_\mathrm{Ł}(x, y)=(x+y)\wedge 1$ \\
Drastic & \index{tconorm\_drastic()}\Rfunc{tconorm\_drastic()} & $$S_\mathrm{D}(x, y)=\left\{
\begin{array}{ll}
1 & \text{if }x, y\in[0,1)\\
x\vee y & \text{if }x=1\text{ or }y=1\\
\end{array}
\right.$$ \\
Fodor & \index{tconorm\_fodor()}\Rfunc{tconorm\_fodor()} & $$S_\mathrm{F}(x, y)=\left\{
\begin{array}{ll}
1 & \text{if }x+y\ge 1\\
